...
3. **개념 Check의 출력 형식은 반드시 아래와 같아야 합니다:**
...
4. **중요: questions 배열의 각 항목은 반드시 객체(object) 형태여야 합니다.**
   - ✅ 올바른 형태: {{"question": "...", "answer": "..."}}
   - ❌ 잘못된 형태: ["질문1", "질문2", "질문3"] (문자열 배열 금지)

5. **개념 Check의 각 question 객체는:**
...
9. 마크다운 코드 블록(```)을 사용하지 마세요.
10. 문서의 구조를 정확하게 파악하여 계층적으로 표현해주세요.
11. **'사회적 희소가치', '상징', '상황 정의'** 등 본문 외곽에 위치한 **용어 정의** 박스는 추출 대상에서 제외해주세요.
12. **'개념 플러스'는 반드시 추출에 포함**하며, 이를 **s_title** 또는 **ss_title** 중 적절한 계층에 넣어주세요.
13. **'contents' 필드에 여러 항목이 나열될 경우, 슬래시(/)를 구분자로 사용하지 마세요.**
14. **'contents' 필드의 여러 문장이나 목록 항목은 텍스트 내에서 반드시 불릿 포인트(•)와 줄 바꿈(\\n)을 사용하여 원본 문서와 같이 목록 형태로 표현해주세요. 각 항목은 독립된 줄에 위치해야 합니다.** (예: "• 첫 번째 항목\\n• 두 번째 항목")
15. 모든 텍스트 내용을 빠짐없이 포함해주세요.
16. s_title, ss_title 계층이 분리되는게 누락되지 않도록 해주세요.
17. ss_title 계층을 인식하지 못하고 s_title의 contents가 두텁게 텍스트를 추출하는 것에 주의해줘. ① 몰가치성, ② 존재 법칙 같은 숫자기호는 ss_title 계층으로 분리해야 하는 것을 누락되지 않게 신경써줘.
